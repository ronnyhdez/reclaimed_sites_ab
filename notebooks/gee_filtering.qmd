...
```{python}
# Check intersections abandoned_wells and fires
# Function to find intersections between buffered polygons and waterbodies
def define_intersection(well):
    intersects_fires = fires.filterBounds(well.geometry()).size().gt(0)
    return well.set('intersects_firess', intersects_fires) 
...
```{python}
check = wells_with_intersections.filter(ee.Filter.eq("intersects_firess", 1))
sample = check.limit(2).getInfo()
print(json.dumps(sample, indent = 2))
```