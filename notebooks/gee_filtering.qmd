...
sample = intersecting_features.limit(10).getInfo()
print(json.dumps(sample, indent=2))
...
wells_intersecting_map.addLayer(
    intersecting_features, {}, 'wells intersecting water')
wells_intersecting_map.addLayer(water_bodies_vector, {'color':'blue'}, 'waterbodies_mask')
wells_intersecting_map
```

# Non-intersecting observations
...
sample = intersecting_features.limit(10).getInfo()
print(json.dumps(sample, indent=2))
total = intersecting_features.size().getInfo()
```

The total number of elements without an intersection: `python print(total)`