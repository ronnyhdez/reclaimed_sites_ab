...
## Run LEAF-toolbox on biggest abandoned well polygons 
...
print(sitesDictionaryL08SR['projects/ee-ronnyale/assets/random_sample_1000_filtered_polygons'])
```

```{python}
# Trying to convert to pandas dataframe
records = []

for site, entries in sitesDictionaryL08SR.items():
    for entry in entries:
        feature = entry['feature']
        time_start = feature['timeStart']
        df = entry['leaftoolbox.SL2PV0']
...
for site, entries in sitesDictionaryL09SR.items():
    for entry in entries:
        feature = entry['feature']
        time_start = feature['timeStart']
        df = entry['leaftoolbox.SL2PV0']
...
combined_df = pd.concat(records, ignore_index=True)
# print(combined_df.head())
pickle_filename = 'time_series_l09sr.pkl'
with open(pickle_filename, 'wb') as file:
    pickle.dump(combined_df, file)
```
...
for site, entries in sitesDictionaryHarmonized.items():
    for entry in entries:
        feature = entry['feature']
        time_start = feature['timeStart']
        df = entry['leaftoolbox.SL2PV0']
...
combined_df = pd.concat(records, ignore_index=True)
# print(combined_df.head())
pickle_filename = 'time_series_harmonized.pkl'
with open(pickle_filename, 'wb') as file:
    pickle.dump(combined_df, file)
```
...
## Other notes:
 ...
 - Need to include start & end dates to each feature.
