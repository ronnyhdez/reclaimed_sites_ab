...
```{python}
wells_df = pl.read_json('vector_data/wells_export.geojson')
buffers_df = pl.read_json('vector_data/buffers_export.geojson')
...
Now we can have a glimpse of the dataframe: 

```{python}
wells_flat.glimpse()
```

### Buffers shape and variables

The same process was applied to the buffers, here are the
results:
...
During the process in GEE, the land use cover were transformed
using numbers. The corresponding categories are:
...
Variables will be transformed with those names and the rest
of variables will have the official names.
...
Let's check first both cleaned datasets: 
...
And some descriptive statistics of the selected sites:

```{python}
melted_wells = wells_flat.melt(
    id_vars = ['well_site'],
    value_vars = ['area_forest', 'area_wetland', 'area_crop_herbaceous', 'area_other'],
    variable_name = 'category_area',
    value_name = 'area'
).with_columns(pl.lit('well').alias('source'))

melted_wells.glimpse()
```

```{python}
melted_buffers = buffers_flat.melt(
    id_vars = ['well_site'],
    value_vars = ['area_forest', 'area_wetland', 'area_crop_herbaceous', 'area_other'],
    variable_name = 'category_area',
    value_name = 'area'
).with_columns(pl.lit('buffer').alias('source'))

melted_buffers.glimpse()
```

```{python}
# Default concat is vertical
combined = pl.concat([melted_buffers, melted_wells])
combined.glimpse()
```
...
sns.barplot(data=combined, x='category_area', y='area', hue='source')
plt.show()
```

Even though the plot shows huge differences, that's because the
reference buffers are bigger than the wells, so I need to
normalize the data.
